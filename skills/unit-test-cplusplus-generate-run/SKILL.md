...
**让 skill 自动生成一个最小化 CMakeLists.txt**: 则由你(claude)生成一个CMakeLists.txt.
...
3. 写 `.test/summary.md`(人类可读),章节:
   ...
   - 跳过目录
4. 拷贝 `summary.md` 到 `test/generated_unit/` 目录:

```bash
cp .test/summary.md test/generated_unit/summary.md
```
...
3. 函数不在本次处理范围(文件被 `coverage_config.exclude_dirs` 排除)
