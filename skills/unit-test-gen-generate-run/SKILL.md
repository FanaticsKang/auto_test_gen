---
name: unit-test-gen-generate-run
description: 单测生成流水线的生成-执行阶段：读取 init 阶段产出的只读基线 `test_cases.json`，通过文件级调度模型派发 Claude sub-agent 生成测试代码、执行测试、采集覆盖率、迭代补测。主 agent 负责调度编排和结果收集，sub-agent 负责生成-执行-覆盖率循环。所有运行状态写到 `generate_process.json` 和 `test_run_state.json`，**基线文件全程只读**。
---

# unit-test-gen-generate-run
...
| `test/generated_unit/<src>/test_<name>.cpp` | 子 agent | 生成的 C++ 测试 |
...
1. **语言对应的 agent 工作流**的完整内容：
   - Python 文件 → `agents/python-test-gen-agent.md`
   - C++ 文件 → `agents/cpp-test-gen-agent.md`
   - 语言从基线 `languages` 字段或文件扩展名判定
...
- C++ 测试：`cmake`、`gcov`、`lcov`

## 参考文档

- `agents/python-test-gen-agent.md`：Python 子 agent 完整工作流定义
- `agents/cpp-test-gen-agent.md`：C++ 子 agent 完整工作流定义
