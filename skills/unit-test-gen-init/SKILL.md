...
在执行扫描之前，检测仓库语言并按需安装依赖。

1. **检测仓库语言**：用 Glob 检查仓库中的源文件
   - Python 文件：`**/*.py`
   - C++ 文件：`**/*.cpp`、`**/*.h`、`**/*.hpp`
2. **按需安装依赖**：
   - 仅 Python 文件 → 无需安装任何依赖（Python 扫描仅使用标准库）
   - 含 C++ 文件 → 检查 tree-sitter 是否已安装：
     ```bash
     python3 -c "import tree_sitter_cpp; import tree_sitter" 2>/dev/null && echo "OK" || echo "MISSING"
     ```
     若输出 `MISSING`，执行安装：
     ```bash
     pip3 install tree-sitter tree-sitter-cpp
     ```

### 步骤 2: 扫描仓库

```bash
python .claude/skills/unit-test-gen-init/scripts/scan_repo.py <repo_root> --output .test/scan_result.json
```
...
```bash
python .claude/skills/unit-test-gen-init/scripts/list_dirs.py --scan .test/scan_result.json
```
...
```bash
python .claude/skills/unit-test-gen-init/scripts/build_baseline.py --scan .test/scan_result.json --output test/generated_unit/test_cases.json --mode full --exclude-dirs <exclude_dirs>
```

#### 4b:增量生成(基线已存在)

```bash
python .claude/skills/unit-test-gen-init/scripts/build_baseline.py --scan .test/scan_result.json --output test/generated_unit/test_cases.json --exclude-dirs <exclude_dirs>
```
...
- Python 3.9+
- C++ 扫描：需要 `tree-sitter` 和 `tree-sitter-cpp`（已在步骤 1 中按需自动安装）
