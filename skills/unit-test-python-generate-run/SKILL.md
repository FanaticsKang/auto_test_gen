...
- 并发度由 AIMD 算法自动调节，遇到 rate limit 会自动降速
...
然后使用 `AskUserQuestion` 工具向用户确认，示例：
...
如果用户选择修改，用 Edit 工具修改 `generate_process.json` 的 `coverage_config` 字段。**不修改 `test_cases.json`**。后续所有脚本和子 agent 均从 `generate_process.json` 读取阈值。

### 步骤 4：原子 claim 一批文件

```bash
python .claude/skills/unit-test-python-generate-run/scripts/dispatch.py claim \
