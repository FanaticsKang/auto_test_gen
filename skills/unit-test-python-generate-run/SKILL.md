...
- 依赖安装：`pip install pytest pytest-cov coverage pytest-xdist psutil`；其中 pytest-xdist + psutil 为并行加速的可选依赖，sub-agent 首次检测到缺失时会尝试自动安装，安装失败则降级为串行
...
| 脚本 | 子命令 | 使用者 | 说明 |
|------|--------|--------|------|
| `dispatch.py` | `init` / `batch` / `claim` / `prepare-shard` / `verify-artifacts` / `report` | 主 agent | 调度编排 + task_envelope 生成 + 产物校验 + 报告生成 |
| `runner.py` | `run` | 子 agent | 测试执行 + 覆盖率采集（支持单文件作用域、增量模式） |
| `analyze.py` | `update-state` / `apply-and-run` / `classify-failures` / `decide-next` / `scaffold-cases` / `lint-cases` / `extract-failures` / `gaps` / `record-bug` / `merge-state` / `merge-bugs` | 子 agent（前 6 个）/ 主 agent（merge-*） | 状态管理 + 执行编排 + 失败分类 + 决策 + 脚手架 + 校验 + shard 合并 |
...
```
步骤 1  确认覆盖率阈值（可与用户交互修改）
步骤 2  安装测试依赖（pytest / pytest-cov / pytest-xdist）
步骤 3  初始化调度状态（生成 generate_process.json）
...
**注意事项：**
- 覆盖率阈值可在步骤 1 修改（默认：语句 90%、分支 90%、函数 100%）
